 ...
 */
import { BULK_MAX_PIXELS, CANVAS_ID, API_VERSION, price, } from '@canvas2026/shared';
export const TILE_SIZE = 100; // matches the server's sharding; only affects batching
export class BudgetExceededError extends Error {
    requestedUnits;
    remainingUnits;
    constructor(requestedUnits, remainingUnits) {
...
export class PixelTooExpensiveError extends Error {
    ...
    priceUnits;
    ceilingUnits;
    constructor(pixel, priceUnits, ceilingUnits) {
...
export class CanvasClient {
    ...
    async quote(pixels) {
        ...
        if (!res.ok)
            throw new Error(`/api/quote ${res.status}`);
     ...
     */
    async paintPixels(pixels, options = {}) {
        const outcome = { painted: [], skipped: [], spentUnits: 0, receipts: [] };
        ...
        for (const chunk of chunkByTile(pixels, this.maxPerRequest)) {
            ...
            for (let attempt = 0; attempt < attemptsPerChunk && remaining.length > 0; attempt++) {
                ...
                if (result.receipt)
                    outcome.receipts.push(result.receipt);
    ...
    async paintChunk(pixels, options) {
        const body = {
            canvas: CANVAS_ID,
            version: API_VERSION,
            pixels,
        ...
        if (ceiling !== undefined) {
            ...
            if (tooDear.length > 0) {
                ...
                const skipped = tooDear.map((t) => ({
                    pixel: pixels.find((p) => p.x === t.x && p.y === t.y),
                    reason: `above per-pixel ceiling (${t.price_units} > ${ceiling})`,
                }));
                if (affordable.length === 0)
                    return { painted: [], skipped, retry: [], waitMs: 0, spentUnits: 0 };
        ...
        // 3. sign and resubmit
        if (!this.opts.signer)
            throw new Error('no signer configured: this client can read and quote, not paint');
    ...
    async handleFailure(res, pixels) {
    ...
    async buildPayment(offer) {
        const signer = this.opts.signer;
        const chainId = Number(offer.requirements.network.split(':')[1]);
        if (!Number.isInteger(chainId))
            throw new Error(`cannot read chain id from network "${offer.requirements.network}"`);
        ...
        const extra = offer.requirements.extra;
...
// ---------------------------------------------------------------- helpers
async function readOffer(res) {
    const body = (await res.json());
    const requirements = body.accepts[0];
    if (!requirements)
        throw new Error('402 response carried no payment requirements');
    const extra = requirements.extra;
    if (!extra.qid || !extra.quote)
        throw new Error('402 response is missing extra.qid / extra.quote');
 ...
 */
export function chunkByTile(pixels, maxPerRequest) {
    const tiles = new Map();
    for (const p of pixels) {
    ...
    for (const group of tiles.values()) {
        for (let i = 0; i < group.length; i += maxPerRequest)
            chunks.push(group.slice(i, i + maxPerRequest));
    }
    return chunks;
}
/** Cost of painting these pixels at their current repaint indices. */
export function estimateUnits(metas) {
