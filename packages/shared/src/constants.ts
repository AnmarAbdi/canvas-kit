...
export function price(n: number): number {
  ...
  for (let i = 0; i < n; i++) {
    p *= 2;
    if (p >= PLATEAU_UNITS) return PLATEAU_UNITS;
  }
  return p;
}

/** Total price for a list of repaint indices. Integer sum; no floats. */
export function priceTotal(ns: readonly number[]): number {
  let total = 0;
  for (const n of ns) total += price(n);
  return total;
}
...
export function immunityMsAt(committedAt: number): number {
...
export function immuneUntil(committedAt: number): number {
...
export function isFrozen(committedAt: number): boolean {
