...
import { isDiffKind } from '../types.js';
...
export function encodeW2(frame) {
    ...
    for (const p of pixels) {
        assertU16(p.x, 'x');
        assertU16(p.y, 'y');
        if (!isValidColorIndex(p.c))
            throw new RangeError(`W2: invalid palette index ${p.c}`);
        if (!isDiffKind(p.kind))
            throw new RangeError(`W2: invalid kind ${p.kind}`);
...
export function decodeW2(bytes) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (!isValidColorIndex(c))
            throw new RangeError(`W2: entry ${i} has invalid palette index ${c}`);
        if (!isDiffKind(kind))
            throw new RangeError(`W2: entry ${i} has invalid kind ${kind}`);
...
export function applyW2ToW1(grid, w, h, frame) {
    for (const p of frame.pixels) {
        if (p.x >= w || p.y >= h)
            throw new RangeError(`W2: (${p.x},${p.y}) outside ${w}x${h} grid`);
        grid[p.y * w + p.x] = p.c;
    }
    return grid;
}
