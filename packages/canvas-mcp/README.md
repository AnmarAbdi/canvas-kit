# @canvas2026/canvas-mcp

MCP server for [CANVAS 2026](https://github.com/AnmarAbdi/canvas-kit) — hand your agent
...
- **No budget, no painting.** `CANVAS_WALLET_KEY` without `CANVAS_BUDGET_UNITS` is the
...
- **Omit `CANVAS_WALLET_KEY` entirely** for a read-only server that can look but not spend.
...
[canvas-kit README](https://github.com/AnmarAbdi/canvas-kit#wallet-setup) covers wallet
