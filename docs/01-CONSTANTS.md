...
| Scheme | `exact` | |
| Network (prod) | `base` | |
| Network (staging) | `base-sepolia` (Circle faucet test USDC) | |
| Asset (prod) | USDC on Base: `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913` | |
| Asset (staging) | USDC on Base Sepolia | `[VERIFY]` current contract address from Circle docs at implementation time |
| Facilitator | Coinbase CDP (fee-free USDC/Base) | `[VERIFY]` current facilitator URL + auth requirements from CDP docs. URL is config-swappable (`FACILITATOR_URL` secret/env), never hardcoded. |
